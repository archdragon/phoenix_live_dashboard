defmodule Phoenix.LiveDashboard.HomeLiveTest do
  ...
  test "shows system information" do
    ...
    assert rendered =~
      ~s|<h6 class="banner-card-title">Dashboard</h6><div class="banner-card-value">#{Application.spec(:phoenix_live_dashboard, :vsn)}</div></div>|
      #~s|"#{Application.spec(:phoenix_live_dashboard, :vsn)}|
    assert rendered =~ ~r"Atoms\s+</div><div><small class=\"text-muted pr-2\">\s+\d+ / \d+\s+</small><strong>\s+\d+%"
    assert rendered =~ ~r"Ports\s+</div><div><small class=\"text-muted pr-2\">\s+\d+ / \d+\s+</small><strong>\s+\d+%"
    assert rendered =~ ~r"Processes\s+</div><div><small class=\"text-muted pr-2\">\s+\d+ / \d+\s+</small><strong>\s+\d+%"
  end
